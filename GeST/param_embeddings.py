...
if __name__ == "__main__":
    ...
    which_folder = {"val": "val/", "train": "train/", "test": "test/", "bug": "bug/", "hard_msp": "hard_msp/", "from_observation": "from_observation/", "param": "param/"}
    ...
    path_merge = argsy['path']+"/images/param/"
    ...
    for _thr in range(1):
        ...
        for i,filename in enumerate(sorted(images)):
            ...
            debut=time.time()
            Gn2v = node2vec.Graph(Gr, False, 1, 2)
            ...
            helper._savefig(segmentation, image, path_figs+str(i+1)+"_"+filename[:-4]+"_"+str(selected_k)+"_STRUCTURAL.png")
            ...
            helper._savefig(segmentation, image, path_figs+str(i+1)+"_"+filename[:-4]+"_"+str(selected_k)+"_COMMUNITY.png")
            #print("metrics computed in {} seconds".format(end-debut))
            ...
            debut=time.time()
            Gn2v = node2vec.Graph(Gr, False, 1, 1)
            Gn2v.preprocess_transition_probs()
            walks = Gn2v.simulate_walks(80, 10)
            model=learn_embeddings(walks,dimensions=16)
            end=time.time()
            ...
            helper._savefig(segmentation, image, path_figs+str(i+1)+"_"+filename[:-4]+"_"+str(selected_k)+"_BALANCED.png")
            ...
            print(filename,PRIAT[i],PRIFV[i],PRINV[i],sep=' ')
