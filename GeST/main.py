...
if __name__ == "__main__":
    ...
    FELZ_PRI, BSR_PRI, LOUVAIN_PRI, GEST_PRI, GEST_EMB_PRI, EMB_PRI, BSL_PRI, LLE_PRI, HOPE_PRI, HOPE_EMB_PRI, VOI, MSE = [], [], [], [], [], [], [], [], [], [], [], []
    ...
    for (dirpath, dirnames, filenames) in walk(path_images):
        for i,filename in enumerate(filenames):
            if filename.endswith(".jpg"):
                ...
                # FIXME: LOAD GRAPH WITH NETWORKX FOR BETTER COMPUTING TIME
                    ...
                    if(argsy['read'] == "True"):
                        ...
                        if(numpy.amax(labels)>1):
                            ...
                            Gr = _distance_r_graph(G,distance,adjacency=adjacency,threshold=0)
                ...
                print("GeST:",max_nb_clusters,k_pri,1-k_pri,mean(tmpmse),mean(tmpvoi),mean(GEST_PRI),mean(VOI),mean(MSE))
