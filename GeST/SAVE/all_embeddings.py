...
from utils.embeddings.embeddings import deepwalk
...
def _apply_clustering(NUM_CLUSTER, embeddings,image,labels,method="",groundtruth=None,GT=None,verbose=False):
    ...
    #clustering = cl.KMeans(init = "k-means++", n_clusters = NUM_CLUSTER, n_init = 35)
    #clustering.fit(data)
    ...
    #clustering = cl.AgglomerativeClustering(n_clusters=None,affinity='euclidean',distance_threshold=2,linkage='average').fit(data)
    clustering = cl.AgglomerativeClustering(n_clusters=NUM_CLUSTER,affinity='euclidean',linkage='average',distance_threshold=None).fit(data)
    ...
    #            'merges': merges}, \
    #return pri, \
    return           labels_from_clustering, numpy.amax(labels_from_clustering)
...
if __name__ == "__main__":
    ...
    FELZ_PRI, BSR_PRI, LOUVAIN_PRI, GEST_PRI, EMB_PRI = [], [], [], [], []
    ...
    for x in range(1):
        for (dirpath, dirnames, filenames) in walk(path_test):
            for i,filename in enumerate(filenames):
                if filename.endswith(".jpg"):
                    ...
                    numSegments = 200
                    gt_boundaries, gt_segmentation = get_groundtruth(gt_test+filename[:-4]+".mat")
                    ...
                    distance=3
                    Gr = _distance_r_graph(G,distance,adjacency=adjacency,threshold=10+(x/10))
                    ...
                    segmentation,_=_apply_clustering(best_k,numpy.asarray(emblle),image,labels,method="Node2Vec",groundtruth=gt_segmentation[0], GT=gt_segmentation)
                    labels_merge,has_merged = merge(segmentation,image_lab,adjacency)
                    #labels_merge=segmentation
                    #has_merged=False
                    pri = probabilistic_rand_index(gt_segmentation,labels_merge)
                    nb_clusters = numpy.amax(labels_merge)
                    #    if(pri > max_pri):
                    #        max_pri = pri
                    #        max_nb_clusters = numpy.amax(labels_merge)
                    max_pri=pri 
                    ...
                    print(max_nb_clusters,max_pri,mean(GEST_PRI))
                    if(has_merged):
                        print("merged")'''

                    # HOPE
                    '''hope = HOPE(d=32,beta=0.01)
                    Y, t = hope.learn_embedding(graph=Gr, edge_f=None, is_weighted=False, no_python=True)
                    embhope = hope.get_embedding()
                    max_pri = 0
                    max_nb_clusters = 0
                    #best_k=0
                    for best_k in NUM_CLUSTERS:
                        segmentation,nb_clusters=_apply_clustering(best_k,numpy.asarray(embhope),image,labels,method="HOPE",groundtruth=gt_segmentation[0], GT=gt_segmentation)
                    ...
                    print(max_nb_clusters,max_pri,mean(EMB_PRI))'''

                    # Node2Vec
                    n2v = node2vec.Graph(Gr,False,1,0.2)
                    n2v.preprocess_transition_probs()
                    walks = n2v.simulate_walks(40,20)
                    model=learn_embeddings(walks,dimensions=16,window_size=5)
                    embn2v=[]
                    ...
                    for best_k in NUM_CLUSTERS:
                        segmentation,_=_apply_clustering(best_k,numpy.asarray(embn2v),image,labels,method="Node2Vec",groundtruth=gt_segmentation[0], GT=gt_segmentation)
                        #labels_merge,has_merged = merge(segmentation,image_lab,adjacency)
                        labels_merge=segmentation
                        has_merged=False
                        pri = probabilistic_rand_index(gt_segmentation,labels_merge)
                    ...
                    for best_k in NUM_CLUSTERS:
                        segmentation,nb_clusters=_apply_clustering(best_k,numpy.asarray(wm.embedding),image,labels,method="HOPE",groundtruth=gt_segmentation[0], GT=gt_segmentation)
                        pri = probabilistic_rand_index(gt_segmentation,segmentation)
                        if(pri > max_pri):
                            max_pri = pri
                            max_nb_clusters = nb_clusters
        ...
        print("{} GeST PRI: {} {}".format(9+(x/10),mean(GEST_PRI),stdev(GEST_PRI)))
        #print("Felzenszwalb PRI: {} {}".format(mean(FELZ_PRI),stdev(FELZ_PRI)))
