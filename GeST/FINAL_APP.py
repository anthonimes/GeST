...
if __name__ == "__main__":
    ...
    for _thr in range(10):
        PRI, VOI = [], []
        stored_PRI_results = dict()
        stored_VOI_results = dict()
        for i,filename in enumerate(sorted(images)):
            ...
            for n_cluster in NUM_CLUSTERS:
                ...
                if(filename in hardimages):
                    ...
                    #segmentation,has_merged=helper._merge_pixels(segmentation,image_lab,thr=0.998,sigma=_sigma)
                    pri = helper._probabilistic_rand_index(gt_segmentation,segmentation)
                ...
                if str(n_cluster) in stored_PRI_results:
                    stored_PRI_results[str(n_cluster)].append(pri)
                else:
                    stored_PRI_results[str(n_cluster)] = [pri]
                    
                if str(n_cluster) in stored_VOI_results:
                    stored_VOI_results[str(n_cluster)].append(mean(tmpvoi))
                else:
                    stored_VOI_results[str(n_cluster)] = [mean(tmpvoi)]
        ...
        GEST_PRI.append(mean(dfPRI.max(axis=1)))
        dfVOI = pd.DataFrame.from_dict(stored_VOI_results)
        GEST_VOI.append(mean(dfVOI.min(axis=1)))
        print(GEST_PRI,GEST_VOI)
    print(max(GEST_PRI),mean(GEST_PRI),mean(GEST_VOI))
