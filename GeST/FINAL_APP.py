        ...
        for _ in range(10):
            for i,filename in enumerate(sorted(filenames)):
                if filename.endswith(".jpg"):
                    ...
                    scaler = StandardScaler()
                    dataemb = scaler.fit_transform(embn2v)
                    ...
                    for n_cluster in NUM_CLUSTERS:
                        ...
                        clustering = cl.AgglomerativeClustering(n_clusters=selected_k,affinity='cosine',linkage='complete',distance_threshold=None).fit(datagest)
                        ...
                        if(filename in hardimages):
                            #helper._savefig(segmentation, image, path_figs+str(i+1)+"_"+filename[:-4]+"_"+str(n_cluster)+"_NOT_MERGED.png")
                        ...
                        selected_k = min(n_cluster,number_regions)
                        clustering = cl.AgglomerativeClustering(n_clusters=selected_k,affinity='cosine',linkage='average',distance_threshold=None).fit(dataemb)
                        labels_clustering = clustering.labels_
                        ...
                        EMB_PRI.append(primerged)
                        
                        segmentation = numpy.zeros(labels.shape,dtype=int)
                    
                        selected_k = min(n_cluster,number_regions)
                        clustering = cl.AgglomerativeClustering(n_clusters=selected_k,affinity='cosine',linkage='average',distance_threshold=None).fit(datafv)
                        labels_clustering = clustering.labels_
