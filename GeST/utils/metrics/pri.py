...
def custom_rand_score(labels_ground_truth, labels_prediction):
    cm = contingency_matrix(labels_ground_truth, labels_prediction)
    sum_tp_fp = comb(cm.sum(0),2).sum()
    sum_tp_fn = comb(cm.sum(1),2).sum()
    tp = comb(cm,2).sum()
    fp = sum_tp_fp - tp
    fn = sum_tp_fn - tp
...
def f_measure(labels_ground_truth, labels_prediction):
