...
if __name__ == "__main__":
    ...
    which_folder = {"val": "val/", "train": "train/", "test": "test/", "hard_msp": "hard_msp/"}
    ...
    #parameters = [(15, 4.5, 20), (8,7,20), (8,5,20)]
    #parameters = [(16, 7, 20), (16,19,40), (6,1.5,20)]
    parameters = [ (8,7,5) ]
    others = [ (40,20) ]
    #PARAMETERS= [ [ list() ]*len(others) for i in range(len(parameters)) ]
    PARAMETERS = [0] * len(parameters)
    for i in range(len(parameters)):
        PARAMETERS[i]=[0]*len(others)
            ...
            if filename.endswith(".jpg"):
                ...
                # FIXME: LOAD GRAPH WITH NETWORKX FOR BETTER COMPUTING TIME
                    ...
                    if(argsy['read'] == "True"):
                        ...
                        _dt=1.2
                        for number_param,(_sr,_rr,_mind) in enumerate(parameters):
                            labels = _meanshift_py(dirpath+filename,_sr,_rr,_mind)
                            number_regions = numpy.amax(labels)
                            for number_other, _other in enumerate(others):
                                #connectivity 2 means 8 neighbors
                                Gr = graph.rag_mean_color(image_lab,labels,connectivity=2,mode='similarity',sigma=_sigma)
                                
                                '''adjacency=nx.to_numpy_matrix(Gr)
                                
                                adjacency[adjacency<=_other]=0
                                ...
                                G=graph.RAG(labels,connectivity=2)
                                distance=0
                                Gr = _distance_r_graph(G,distance,adjacency=adjacency,threshold=_other)'''
                                
                                Gn2v = node2vec.Graph(Gr, False, 2, 0.5)
                                Gn2v.preprocess_transition_probs()
                                walks = Gn2v.simulate_walks(_other[0],_other[1])
                                model=learn_embeddings(walks,dimensions=64)
                                embn2v=[]
                                for node in Gr.nodes():
                                    embn2v.append(model.wv.get_vector(str(node)).tolist())
                                ...
                                PARAMETERS[number_param][number_other].append(k_pri)
                                print(_sr,_rr,_mind,_sigma,number_regions,_other,":",max_nb_clusters,k_pri,mean(PARAMETERS[number_param][number_other]))
    ...
    for i in range(len(parameters)):
        for j,_ in enumerate(others):
            print(parameters[i],others[j],sum(PARAMETERS[i][j])/100)
