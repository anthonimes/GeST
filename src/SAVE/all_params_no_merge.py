...
def merge(labels,similarity,thr_pixels=200,thr_similarity=3.5):
    def _mean_lab():
        mean_lab = []
...
def _apply_clustering(NUM_CLUSTER, embeddings,image,labels,method="",groundtruth=None,GT=None,verbose=False,_dt=None):
    ...
    #clustering = cl.DBSCAN(eps=0.6,metric='euclidean',n_jobs=4).fit(data)
    clustering = cl.AgglomerativeClustering(n_clusters=None,distance_threshold=_dt).fit(data)
    #clustering = cl.AgglomerativeClustering(n_clusters=NUM_CLUSTER,distance_threshold=None).fit(data)
    #clustering = cl.AgglomerativeClustering(n_clusters=15,distance_threshold=None).fit(data)
        ...
        for _threshold in thresholds:
            for _distance in distances:
                for _connectivity in connectivities:
                    for dt in dist_thr:
                        FELZ_PRI, BSR_PRI, LOUVAIN_PRI, GEST_PRI, DW_PRI = [], [], [], [], []
                        ...
                        for (dirpath, dirnames, filenames) in walk(path_val):
                            for i,filename in enumerate(filenames):
                                if filename.endswith(".jpg"):
                                    ...
                                    distance=1
                                    Gr = _distance_r_graph(G,distance,adjacency=adjacency,threshold=10)
                                    ...
                                    #best_k=0
                                    for best_k in NUM_CLUSTERS:
                                        pri, segmentation,nb_clusters=_apply_clustering(best_k,numpy.asarray(embhope),image,labels,method="HOPE",groundtruth=gt_segmentation[i], GT=gt_segmentation)
                                        #if(pri > max_pri):
                                        #    max_pri = pri
                                    HOPE_PRI.append(pri)
                                    #GEST_PRI.append(probabilistic_rand_index(gt_segmentation,segmentation))
                                    print(nb_clusters,mean(HOPE_PRI))'''
                                    ...
                                    #for best_k in NUM_CLUSTERS:
                                    segmentation,nb_clusters=_apply_clustering(best_k,numpy.asarray(embn2v),image,labels,method="Node2Vec",groundtruth=gt_segmentation[0], GT=gt_segmentation, _dt=dt)
                        ...
                        print("compactness: {}\t threshold: {}\t distance: {}\t connectivity: {}\t agglomerative: {}\t PRI: {} {}".format(_compactness,_threshold,_distance,_connectivity,dt,mean(GEST_PRI),stdev(GEST_PRI)))
